...
type SectionProps = { id?: string; title?: React.ReactNode; children?: React.ReactNode };
...
type SealButtonProps = { href: string; children?: React.ReactNode };
type CardProps = { children?: React.ReactNode };
type AdminPanelProps = {
  setHeroBg: (v: string) => void;
  setLogoSrc: (v: string) => void;
  setAboutImg: (v: string) => void;
};
  ...
  return (
    <div className="relative group">
      ...
      <div className="invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-200 absolute right-0 mt-2 w-44 rounded-xl border bg-white/95 shadow-sm backdrop-blur" style={{ borderColor: THEME.accent }}>
        <ul className="py-2">
          {items.map((it: DropdownItem, i: number) => (
            <li key={i}>
...
function SealButton({ href, children }: SealButtonProps) {
  return (
...
function AdminPanel({ setHeroBg, setLogoSrc, setAboutImg }: AdminPanelProps) {
  return (
    <div className="fixed bottom-6 right-6 z-50 rounded-2xl border bg-white/90 backdrop-blur p-3 shadow-sm" style={{borderColor: THEME.accent}}>
      <div className="text-xs mb-2" style={{color: THEME.primary}}>Admin · Quick Switch</div>
      <div className="flex gap-2">
        <button onClick={() => (document.getElementById('hero-file') as HTMLInputElement | null)?.click()} className="rounded-xl border px-3 py-1 text-xs" style={{borderColor: THEME.primaryDark, color: THEME.primaryDark}}>Hero</button>
        <button onClick={() => (document.getElementById('logo-file') as HTMLInputElement | null)?.click()} className="rounded-xl border px-3 py-1 text-xs" style={{borderColor: THEME.crimson, color: THEME.crimson}}>Logo</button>
        <button onClick={() => (document.getElementById('about-file') as HTMLInputElement | null)?.click()} className="rounded-xl border px-3 py-1 text-xs" style={{borderColor: THEME.accentDark, color: THEME.accentDark}}>About</button>
      </div>
      <input id="hero-file" type="file" accept="image/*" className="hidden" onChange={(e)=>{ const f=(e.target as HTMLInputElement).files?.[0]; if(f) setHeroBg(URL.createObjectURL(f)); }} />
      <input id="logo-file" type="file" accept="image/*" className="hidden" onChange={(e)=>{ const f=(e.target as HTMLInputElement).files?.[0]; if(f) setLogoSrc(URL.createObjectURL(f)); }} />
      <input id="about-file" type="file" accept="image/*" className="hidden" onChange={(e)=>{ const f=(e.target as HTMLInputElement).files?.[0]; if(f) setAboutImg(URL.createObjectURL(f)); }} />
    </div>
  );
}
...
export default function TangQingSite() {
  const L = Copy.en;
  const [heroBg, setHeroBg] = useState<string>(HERO_BG);
  const [storyImages] = useState<string[]>([]);
  const [logoSrc, setLogoSrc] = useState<string>(LOGO);
  const [aboutImg, setAboutImg] = useState<string>("/photo1.jpg");
    ...
    <div style={{ background: THEME.paper, fontFamily: "'Noto Serif SC','Songti SC','SimSun','Times New Roman',serif", backgroundImage: "radial-gradient(rgba(0,0,0,0.03) 1px, transparent 1px), linear-gradient(0deg, rgba(200,169,106,0.05), rgba(200,169,106,0.05))", backgroundSize: "10px 10px, auto", backgroundAttachment: "fixed", backgroundBlendMode: "multiply" }} className="min-h-screen text-zinc-900">
        ...
        <div className="mt-6 grid sm:grid-cols-2 gap-5">
          <Card>
            <div className="p-4">
              ...
              <p className="mt-1 text_sm">油胞丰富、香气清扬；皮更紧致，转化更稳。</p>
            </div>
          </Card>
          <Card>
            <div className="p-4">
              <div className="flex items_center gap-2"><MapPin size={16} style={{ color: THEME.accent }} /><strong>Yunnan 云南</strong></div>
      ...
      <Section id="process" title={L.processTitle}>
        <ol className="list-decimal pl-5 space-y-2">
          {L.processSteps.map((s: string, i: number) => (
            <li key={i}>{s}</li>
          ))}
        </ol>
      </Section>
      ...
      <Section id="brew" title={L.brewTitle}>
        <ul className="list-disc pl-5 space-y-2">
          {L.brewPoints.map((s: string, i: number) => (
            <li key={i}>{s}</li>
          ))}
        </ul>
      </Section>
      ...
      <Section id="faq" title={L.faqTitle}>
        <div className="divide-y">
          {L.FAQ.map((item: { q: string; a: string }, i: number) => (
      ...
      {isAdmin && (
        <AdminPanel setHeroBg={setHeroBg} setLogoSrc={setLogoSrc} setAboutImg={setAboutImg} />
      )}
    </div>
  );
}
